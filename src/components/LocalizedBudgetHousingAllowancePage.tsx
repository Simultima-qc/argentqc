...
export default function LocalizedBudgetHousingAllowancePage({ locale }: { locale: Locale }) {
  ...
  const faqSchema = {
    ...
    "@type": "FAQPage",
    mainEntity: dictionary.faqs.map((faq) => ({
      "@type": "Question",
      name: faq.question,
      acceptedAnswer: {
        "@type": "Answer",
  ...
  return (
    <main style={{ minHeight: "100vh", background: PARCH }}>
      ...
      <SiteFooter
        ...
        locale={locale}
        contentClassName="mx-auto max-w-lg text-center"
        style={{ background: DARK }}
        legalStyle={{ color: "rgba(240,235,224,0.3)" }}
      />
    </main>
  );
}
